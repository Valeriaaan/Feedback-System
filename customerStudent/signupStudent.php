...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate inputs
    if (empty($studentID) || empty($department) || empty($password) || empty($confirmPassword)) {
        $error = "All fields are required.";
    } elseif ($password !== $confirmPassword) {
        $error = "Passwords do not match.";
    } else {
        // Check if studentID already exists
        $check_stmt = $conn->prepare("SELECT studentID FROM student WHERE studentID = ?");
...
<head>
    ...
    <title>Student Sign-Up - CNSC Customer Feedback System</title>
                ...
                <div class="card p-4">
                    ...
                    <form id="signupForm" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                        ...
                        <div class="text-center mt-3">
                            <p>Already a student? <a href="loginStudent.php">Login Here!</a></p>
                        </div>
                    </form>
                </div>
            </div>
